...
export async function handleGetSwitchOptions(request, env) {
	try {
		const switches = await env.DB
			.prepare("SELECT id, name, price, count, fire_type, networkable, protocol, color, created_at, updated_at FROM switch_options ORDER BY id")
			.all();
...
export async function handleAddSwitchOption(request, env) {
	try {
		...
		const { name, price = 0.0, count, fireType = '', networkable = false, protocol = '', color = '' } = body;
		await env.DB
			.prepare("INSERT INTO switch_options (name, price, count, fire_type, networkable, protocol, color) VALUES (?, ?, ?, ?, ?, ?, ?)")
			.bind(name, price, count, fireType, networkable ? 1 : 0, protocol, color)
			.run();
...
export async function handleUpdateSwitchOption(request, env) {
	try {
		...
		const { id, name, price = 0.0, count, fireType = '', networkable = false, protocol = '', color = '' } = body;
		await env.DB
			.prepare("UPDATE switch_options SET name = ?, price = ?, count = ?, fire_type = ?, networkable = ?, protocol = ?, color = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?")
			.bind(name, price, count, fireType, networkable ? 1 : 0, protocol, color, id)
			.run();
